    ...
    <Dialog>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="flx items-center gap-2 select-none">
        ...
        <DropdownMenuContent align="end" className="w=56">
          <DropdownMenuLabel className='flex flex-col'>
            {isLoadingProfile ? <div className='space-y-1.5'>
              <Skeleton className="h-4 w-32"/>
              <Skeleton className="h-3 w-24"/>
            </div> :
            <>
              <span>{profile?.name}
              {isLoadingProfile ? <Skeleton className='h-4 w-40'/> : managedRestaurant?.name}
              </span>
