...
export const StoreProfileDialog = () => {
  ...
  const {mutateAsync: updateProfileFn} = useMutation({
    ...
    onMutate({name, description}){
      ...
      return {preiousProfile: cached}
    },
    onError(_,__,context){
      if(context?.preiousProfile){
        updateManagedRestaurantCache(context.preiousProfile)
      }
    }
  })

  async function handleFormSubmit(data: StoreProfileSchema){
    try {
